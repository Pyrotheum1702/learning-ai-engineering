# 📋 To-Do Projects

A backlog of candidate builds — real briefs, gigs, and ideas I want to take on. Each gets its own folder with a README covering the brief, goal, stack, and a build plan. When I start one, I move it toward `🟨 In progress`; when shipped, it graduates to a regular entry in [`../`](../).

Status legend: ⬜ Not started · 🟨 In progress · ✅ Done

| Project | Status | Stack | Notes |
|---------|--------|-------|-------|
| [Multi-Tenant WhatsApp AI Agent](whatsapp-ai-agent/) | ⬜ | FastAPI · LangGraph · PostgreSQL | WhatsApp Business API chatbot with LLM |
| [Clinical Report Extraction](clinical-report-extraction/) | ⬜ | LLMs · structured output · clinical NLP | Extract structured data from pathology reports & notes (biotech) |
