...
| Date | Topic | Notes |
|------|-------|-------|
| 2026-07 | Python fundamentals | Working through [20 build-it-yourself projects](projects/learning-python/) — shipped #1 (number guessing), #2 (unit converter), and #3 (command-line to-do list with JSON persistence); learned EAFP input handling, dict dispatch, and the `method` vs `method()` / immutable-string-return gotchas |
...
| [learning-python](projects/learning-python/) | 🟨 | 20 build-it-yourself Python projects, easy → hard (3/20 done). Fundamentals → data & persistence → real-world tooling → systems, async & packaging. |
